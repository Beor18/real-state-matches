...
function transformToDatabase(
  ...
  index: number,
): DatabasePropertyInsert {
  // Determine if it's for rent or sale
  const isRent =
    xposure.title?.toLowerCase().includes("alquiler") ||
  ...
  const addressStr =
    xposure.address?.trim() ||
    `${xposure.stName || ""} ${xposure.stNum || ""}`.trim();
  ...
  if (xposure.bedrooms)
    descriptionParts.push(`${xposure.bedrooms} habitaciones.`);
  if (xposure.bathrooms) descriptionParts.push(`${xposure.bathrooms} baños.`);
  if (xposure.sqft_total)
    descriptionParts.push(`${xposure.sqft_total} pies cuadrados.`);
  ...
  if (
    subdivision.toLowerCase().includes("urbanizacion") ||
    subdivision.toLowerCase().includes("urbanización")
  ) {
...
async function main() {
  ...
  if (deleteError) {
    console.error(
      "⚠️  Warning: Could not delete old properties:",
      deleteError.message,
    );
  } else {
