...
class XposureClient {
  ...
  async searchNormalized(
    ...
    try {
      ...
      if (params.city) {
        ...
        console.log(
          `Xposure: Searching for city "${params.city}" with variations:`,
          cityVariations,
        );
      ...
      console.log(
        `Xposure: Found ${
          properties.length
        } properties (total: ${count}) for city: ${params.city || "any"}`,
      );
    ...
    try {
      ...
      return {
        ...
        message: `Conexión exitosa. ${
          count || 0
        } propiedades de Xposure en la base de datos.`,
      };
    } catch (error) {
      const message =
  ...
  transformDatabaseToNormalized(
    ...
    return {
      ...
      title:
        dbProperty.title ||
        `${propertyTypeToSpanish(
          dbProperty.property_type,
        )} en ${listingTypeText} - ${dbProperty.city}`,
      description: `Propiedad en ${
        dbProperty.neighborhood || dbProperty.city
      }, ${dbProperty.state}. ${dbProperty.bedrooms} habitaciones, ${
  ...
  static transformXposureToNormalized(
    xposureProperty: XposureProperty,
  ): NormalizedProperty {
    ...
    const isRent =
      xposureProperty.title?.toLowerCase().includes("alquiler") ||
      !!xposureProperty.price_current_rent;

    const price = isRent
      ? parsePrice(xposureProperty.price_current_rent)
      : parsePrice(xposureProperty.price_current) ||
        parsePrice(xposureProperty.price_sold);
    ...
    const propertyType = propertyTypeFromIcon(
      xposureProperty.property_icon || "",
    );
    ...
    return {
      ...
      description: `Propiedad en ${neighborhood || city}, Puerto Rico. ${
        xposureProperty.subdivision
          ? `Urbanización: ${decodeHtml(xposureProperty.subdivision)}`
          : ""
      }`,
      price,
      address: {
        street:
          xposureProperty.address?.trim() ||
          `${xposureProperty.stName} ${xposureProperty.stNum}`.trim(),
        city,
        state: "PR",
        zipCode: "",
        country: "PR",
      ...
      coordinates:
        xposureProperty.lat && xposureProperty.lng
          ? {
      ...
      features: xposureProperty.parking_spaces
        ? [`${xposureProperty.parking_spaces} estacionamientos`]
        : [],
      amenities: [],
      images: xposureProperty.thumbnailPhotoURL
        ? [xposureProperty.thumbnailPhotoURL]
        : [],
