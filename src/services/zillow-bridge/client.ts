...
class ZillowBridgeClient {
  ...
  private async request<T>(
    ...
    console.log(
      `[Bridge API] URL: ${url.replace(/access_token=[^&]+/, "access_token=***")}`,
    );
  ...
  async searchListings(
    ...
    if (params.propertyType) {
      ...
      if (typeFilter) {
        ...
        filters.push(
          `contains(tolower(PropertyType), '${params.propertyType.toLowerCase()}')`,
        );
      }
    }
    ...
    if (params.sortBy) {
      ...
      odataParams.push(
        `$orderby=${encodeURIComponent(`${sortField} ${sortDir}`)}`,
      );
    } else {
      odataParams.push(
    ...
    try {
      ...
      const properties = response.value.map((p) =>
        this.transformToNormalized(p),
      );
  ...
  transformToNormalized(bridgeProperty: BridgeProperty): NormalizedProperty {
    ...
    console.log(
      `[Bridge API] Property ${bridgeProperty.ListingKey}: ${bridgeProperty.Media?.length || 0} media items, ${images.length} images extracted`,
    );

    // Calculate total bathrooms
    const bathrooms =
      (bridgeProperty.BathroomsFull || 0) +
        (bridgeProperty.BathroomsHalf || 0) * 0.5 ||
      bridgeProperty.BathroomsTotalInteger ||
      0;
    ...
    return {
      ...
      coordinates:
        bridgeProperty.Latitude && bridgeProperty.Longitude
          ? {
      ...
      agent: bridgeProperty.ListAgentFullName
        ...
        : undefined,
      listDate:
        bridgeProperty.OriginalEntryTimestamp || new Date().toISOString(),
      modifiedDate:
        bridgeProperty.ModificationTimestamp || new Date().toISOString(),
      status:
        statusMap[bridgeProperty.MlsStatus || bridgeProperty.StandardStatus] ||
        "active",
    };
  }
}
