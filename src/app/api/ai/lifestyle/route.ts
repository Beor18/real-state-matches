...
export async function POST(request: NextRequest) {
  ...
  function progress(step: string, message: string, detail: string, percent: number) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        controller.enqueue(progress('init', 'Iniciando busqueda...', 'Verificando configuracion', 5))
        ...
        const budgetStr = budget ? `$${Number(budget).toLocaleString('en-US')}` : 'sin limite'
        controller.enqueue(progress(
          'found',
        ...
        if (propertyResult.properties.length > 0) {
          ...
          // Step 5: AI matching
          const keywordsStr = profileAnalysis.keywords?.slice(0, 4).join(', ') || ''
          controller.enqueue(progress(
            'matching',
            `Evaluando ${propertiesForAI.length} propiedades con IA...`,
        ...
        controller.enqueue(errorEvent('Error al procesar la busqueda de estilo de vida'))
        controller.close()
      }
    },
  })
